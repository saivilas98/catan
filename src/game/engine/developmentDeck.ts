...
import { DEVELOPMENT_DECK_COMPOSITION } from '../models/types';
...
/**
 * Builds and shuffles the 25-card deck. Seeded so a given seed always produces the
 * same draw order, which is what makes development-card tests deterministic.
 * Cards start with acquiredTurnNumber 0; buying stamps the real turn.
 */
export function generateDevelopmentDeck(seed: number = randomSeed()): DevelopmentCard[] {
  const ordered: DevelopmentCardType[] = [];
  for (const [type, count] of Object.entries(DEVELOPMENT_DECK_COMPOSITION)) {
